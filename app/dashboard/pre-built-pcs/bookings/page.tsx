...
import { usePrebuiltOrders, usePrebuiltOrderActions } from "@/components/dashboard/store"
import { PREBUILT_STATUS_OPTIONS, prebuiltOrderFields } from "@/components/dashboard/bookings"
...
export default function PrebuiltBookingsPage() {
  ...
  const [modalOrder, setModalOrder] = useState<PrebuiltOrder | null>(null)
  const [modalMode, setModalMode] = useState<"view" | "edit">("view")
  const [deleteTarget, setDeleteTarget] = useState<PrebuiltOrder | null>(null)

  function handleSave(values: Record<string, string>) {
    if (!modalOrder) return
    const statusOption = PREBUILT_STATUS_OPTIONS.find((o) => o.value === values.status)
    return updatePrebuiltOrder(modalOrder.id, {
  ...
  return (
    <div className="flex flex-col gap-6">
            ...
            {orders.map((order) => (
              ...
              >
                ...
                <StatusDropdown
                  value={order.status}
                  tone={order.tone}
                  options={PREBUILT_STATUS_OPTIONS}
                  onChange={(status, tone) => updatePrebuiltOrder(order.id, { status, tone })}
                />
      ...
      <RecordModal
        ...
        mode={modalMode}
        title={modalMode === "edit" ? "Edit pre-built order" : "Pre-built order details"}
        subtitle={modalOrder?.id}
        fields={modalOrder ? prebuiltOrderFields(modalOrder) : []}
      ...
      <ConfirmDeleteDialog
        ...
        onConfirm={() => (deleteTarget ? removePrebuiltOrder(deleteTarget.id) : undefined)}
